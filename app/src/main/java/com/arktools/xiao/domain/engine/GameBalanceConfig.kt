...
object GameBalanceConfig {
     ...
     * v3.1 数值测试收紧：中期紧平衡（靠学科/竞赛/国际赚外快），满级微平衡（不经营新系统即亏损）
     * Lv.1: 0.52万/月  Lv.2: 0.62  Lv.3: 0.78  Lv.4: 1.04  Lv.5: 1.7  Lv.6: 1.9
     */
    ...
    fun getMonthlyRent(campusLevel: Int): Double {
        ...
        return when (campusLevel) {
            1 -> 4.0      // 新建大学，月运营4万
            2 -> 20.0     // 区域应用型，月运营20万
            3 -> 90.0     // 省级特色，月运营90万（v3.1 收紧中期现金流）
            4 -> 450.0    // 国家一流，月运营450万
            5 -> 1500.0   // 高水平研究型，月运营1500万
            6 -> 2800.0   // 世界一流，月运营2800万（满级微平衡：需持续经营学科/国际）
            else -> 8.0
        }
    }
    ...
    fun getMarketTrend(subject: com.arktools.xiao.domain.model.Subject, theme: com.arktools.xiao.domain.model.CourseTheme, year: Int): Float {
        var trend = 1.0f

        when {
            year < 2000 && theme == com.arktools.xiao.domain.model.CourseTheme.EXAM_PREP -> trend *= 1.3f
            year < 2000 && theme == com.arktools.xiao.domain.model.CourseTheme.INTEREST -> trend *= 0.7f
            year >= 2015 && theme == com.arktools.xiao.domain.model.CourseTheme.STEM -> trend *= 1.2f
            year >= 2020 && subject == com.arktools.xiao.domain.model.Subject.MUSIC -> trend *= 1.1f
            year >= 2020 && theme == com.arktools.xiao.domain.model.CourseTheme.INTERNATIONAL -> trend *= 1.1f
        }
     ...
     * 设施解锁（"可建建筑 X/Y" 的 Y）。
     *
     * 上调过一版：原值 18/28/40/56/80/140 在后期会把玩家卡死——宿舍、食堂、教室、
     * 图书馆这些都要占名额，校园花园/校门/体育馆这类偏装饰的建筑也走同一份额度，
     * 于是出现"学位不够想再建一栋教室，但建筑位已满、而且没有别的办法"的硬墙。
     * 地块本身（Lv4≈986 格、Lv6=1536 格）远没建满，放开额度不会破坏地图尺寸。
     */
     ...
     */
    fun isDistrictUnlocked(district: com.arktools.xiao.domain.model.DistrictType, schoolLevel: Int, reputation: Long): Boolean {
     ...
     * 获取学区的等级名称描述
     */
    fun getDistrictLevelRequirementText(district: com.arktools.xiao.domain.model.DistrictType): String {
    ...
    fun getHiringFee(level: com.arktools.xiao.domain.model.TeacherLevel): Double {
        return when (level) {
            com.arktools.xiao.domain.model.TeacherLevel.C -> 5.0    // C级猎头费5万
            com.arktools.xiao.domain.model.TeacherLevel.B -> 20.0   // B级20万
            com.arktools.xiao.domain.model.TeacherLevel.A -> 60.0   // A级60万
            com.arktools.xiao.domain.model.TeacherLevel.S -> 200.0  // S级200万（顶级猎头费）
        }
    }
}
...
enum class GameModule(
    ...
    EXAM("教学评估", 1, "跟踪培养质量"),
    CLUB("社团活动", 3, "学生社团开始申请"),
    SEASONAL("季节活动", 3, "校园活动日历开放"),
    STUDENT_LIFE("学生生活", 1, "宿舍食堂从开学就要管"),
    TIMETABLE("专业课表", 1, "开班后就能排课"),
    MARKETING("营销推广", 3, "招生传播可以花钱换生源"),
    REPUTATION("声誉详情", 3, "社会口碑开始影响生源质量"),
    SCHOLARSHIP("奖助学金", 1, "开学就能设奖助学金稳住生源"),
