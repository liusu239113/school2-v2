...
sealed class CompetitorEvent {
     ...
     */
    fun toGameEvent(): com.arktools.xiao.domain.model.GameEvent? {
        return when (this) {
            is PriceWar -> com.arktools.xiao.domain.model.GameEvent.NegativeEvent(
                title = "价格战",
                message = "${competitorName}发起价格战，大幅降低学费抢夺生源！学校声誉-${reputationLoss}。",
                penaltyCash = 0.0,
                penaltyReputation = 0L
            )
            is TalentPoaching -> com.arktools.xiao.domain.model.GameEvent.NegativeEvent(
                title = "人才挖角",
                message = "${competitorName}试图高薪挖走你的教师！",
                penaltyCash = 0.0
            )
            is MarketExpansion -> com.arktools.xiao.domain.model.GameEvent.NegativeEvent(
                title = "市场扩张",
                message = "${competitorName}在你的学区大规模投放广告，招生竞争加剧。学校声誉-2。",
                penaltyCash = 0.0,
                penaltyReputation = 0L
            )
            is Partnership -> com.arktools.xiao.domain.model.GameEvent.PositiveEvent(
                title = "合作邀请",
                message = "${competitorName}与你达成合作协议，互惠共赢！学校声誉+${reputationGain}。",
                bonusReputation = 0L
            )
            is CompetitorCollapse -> com.arktools.xiao.domain.model.GameEvent.PositiveEvent(
                title = "对手倒闭",
