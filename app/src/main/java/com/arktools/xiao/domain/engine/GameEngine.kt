...
internal fun isStudentGraduationDue(
    ...
    // 9月入学：三年制第3年6月毕业（跨度3），四年制第4年6月毕业（跨度4）。
...
class GameEngine @Inject constructor(
    ...
    private suspend fun msStage4(school: School, st: MonthlySettlementState) {
            ...
            emitEvent(
                MonthlyPrincipalDecision.build(
                    school = school,
                    studentCount = st.cachedActiveStudentsForMonth.size,
                    latestCash = latestCash,
                    netProfit = netProfit,
                    extraWindows = policyManager.policies.value.collegeDevelopment.buildingOps.extraWindows
                ),
                school
            )
