...
import com.arktools.xiaozhang.domain.model.MilestoneType
...
class EventGenerator @Inject constructor() {
    ...
    private fun getSeasonalEvents(school: School): List<GameEvent> {
        ...
        return when (event) {
            ...
            SemesterCalendar.SchoolEvent.SPORTS_DAY -> {
                if (school.facilities.any { it.type == com.arktools.xiaozhang.domain.model.FacilityType.SPORTS_FIELD }) listOf(
                    GameEvent.PositiveEvent(
                        title = "运动会佳绩",
    ...
    private fun getConnectionEvents(principal: Principal, school: School): List<GameEvent> {
        ...
        val hasOfficialConnection = principal.connections.any {
            it.type == com.arktools.xiaozhang.domain.model.ConnectionType.EDUCATION_OFFICIAL && it.relationLevel > 50
        }
        ...
        val hasMediaConnection = principal.connections.any {
            it.type == com.arktools.xiaozhang.domain.model.ConnectionType.MEDIA_REPORTER && it.relationLevel > 30
        }
        ...
        val hasPoliceConnection = principal.connections.any {
            it.type == com.arktools.xiaozhang.domain.model.ConnectionType.POLICE_CONTACT
        }
    ...
    private fun getFacilityEvents(school: School): List<GameEvent> {
        ...
        // 特定设施触发
        if (school.facilities.any { it.type == com.arktools.xiaozhang.domain.model.FacilityType.LIBRARY }) {
            events.add(GameEvent.ChoiceEvent(
                title = "读书月活动",
        ...
        if (school.facilities.any { it.type == com.arktools.xiaozhang.domain.model.FacilityType.COMPUTER_LAB }) {
            events.add(GameEvent.ChoiceEvent(
                title = "网络安全事件",
        ...
        if (school.facilities.any { it.type == com.arktools.xiaozhang.domain.model.FacilityType.CANTEEN }) {
            events.add(GameEvent.ChoiceEvent(
                title = "食堂卫生抽查",
    ...
    private fun getSocialEvents(school: School): List<GameEvent> {
        ...
        // 社区投诉噪音
        if (school.facilities.any { it.type == com.arktools.xiaozhang.domain.model.FacilityType.SPORTS_FIELD }) {
            events.add(GameEvent.ChoiceEvent(
                title = "居民投诉噪音",
